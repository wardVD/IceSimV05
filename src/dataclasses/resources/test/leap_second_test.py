...
for year in range (1970,max(dataclasses.year_of(leap_sec_mjd[-1]),now.year)+3):
    ...
    if leap_sec:
        ...
        # but its C interface dosn't do bounds checking on the input.
        # So the datetime object you get from a leap second is not the 
        # same as the second afterward but once you apply a timedelta acts just like the second afterward
