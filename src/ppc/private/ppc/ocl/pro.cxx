...
__kernel void propagate(__private uint num,
...
#ifdef TALL
  for(unsigned int i=idx; i<num; i+=get_global_size(0)){
#else
...
#ifdef TALL
      pbuf f; f.r=r, f.n=n, f.q=j; bf[i]=f;
  }
  ...
  for(uint i=idx; i<num; TOT==0 && (XINC)){
    ...
    if(TOT==0){ // initialize photon
      pbuf f=bf[i];
      r=f.r; n=f.n; w=&ez->w[f.q];
      if(e.type<=0) niw=ep[i/OVER].q;
#endif
      om=e.fla;
    ...
    if(om!=-1){
...
#ifdef PDIR
      h.pth=acos(n.z); h.pph=atan2(n.x, n.y);
      ...
      h.dth=acos(clamp(div(dz, e.R), -1.f, 1.f)); h.dph=atan2(dx, dy);
#endif
