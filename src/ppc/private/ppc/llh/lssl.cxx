namespace LSSL{
  ...
  double wref(double * A, double * D, double * V, double * S, int N, int M, double trs = 0, double *as = NULL, double xmin = 0, double xmax = 1, double *XA = NULL, double *XS = NULL){
    ...
    double X[m], Xfl=0; a = new double[n*m]; c = new double[m]; d=D; s=S;
	...
	double vi=V[i]; X[k++]=vi;
	if(vi>0) if(vi<Xfl || Xfl==0) Xfl=vi;
      }
    }
    if(Xfl<=0) Xfl=1; Xfl/=M-m+1; // Xfl=1;
    for(int i=0; i<m; i++){
      double ci=0;
      for(int j=0; j<n; j++) ci+=a[j+n*i];
      c[i]=ci;
      if(trs<0&&X[i]<=0) X[i]=Xfl;
    }
