// Event header for paylaods of V3
...
{
  template<>
    void do_payload_decode<19>(DecodeTarget<19>::type& target,
        const DecodeConfiguration& configuration,
        const vector<char>& eventData,
        unsigned start)
    {
      log_trace("decoding a type 19");
      ...
      if(config.year == 0)
      {
        log_fatal("You have to set the year for this payload type!");
      }
