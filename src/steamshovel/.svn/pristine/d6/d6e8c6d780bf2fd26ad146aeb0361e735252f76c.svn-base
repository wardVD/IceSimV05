...
class IceTopCanvas(FigureCanvas):
    ...
    def __init__(self, settings):
        ...
        self.array_dots = Line2D([],[],
            marker=".", linestyle="None", color="k", alpha=0.5,
            markeredgecolor="None", zorder=1, picker=5)
        ax.add_line(self.array_dots)

        self.array_wedges = PatchCollection([],
            zorder=2, picker=5)
        ax.add_collection(self.array_wedges)
        ...
        for ib in (0, 1):
            ...
            ax.add_line(m)
        self.ldf_points = ax.scatter([],[], zorder=2, picker=1)
        self.ldf_slc_points = ax.scatter([],[], color="None",
        ...
        self.time_points = ax.scatter([], [],zorder=2, picker=2)
        self.time_slc_points = ax.scatter([],[], color="None",
    ...
    def handle_time_trace(self, show_time_trace, tank_data):
        fig = self.figure
    ...
    def update_frame(self, frame, keys, settings):
        ...
        # keep tank_data around, used by on_pick
        rec_data, self.tank_data = get_data(frame, keys, use_log_radius)
        ...
        def pulse_cmp(a, b):
            ...
            if v == 0:
                return int(a["t_hit"] - b["t_hit"])
            return v
        for d in self.tank_data:
            ctr = d["ctr"]
            phi = d["phi"]
            pos = d["pos"]
            x_st.append(pos[0])
            ...
            if pulses:
                ...
                t_tank = pulses[0]["t_hit"]
                ch_tank = pulses[0]["charge"]
                for pulse in pulses:
                    pulse_ind.append(len(self.t_hit))
                    ch = pulse["charge"]
                    r = d["r"]
                    r_hit.append(abs(r) if use_log_radius else r)
                    s_hit.append(ch)
                    self.t_hit.append(pulse["t_hit"])
                    t = pulse["t_plane"]
                ...
                if show_nohits:
                    r_nohit.append(d["r"])
            self.t_tank.append(t_tank)
            self.tank_pulse_index.append(pulse_ind)
            rad = scale * ch_tank ** power
            cp = np.cos(np.radians(phi + 90))
            sp = np.sin(np.radians(phi + 90))
            # workaround for weird matplotlib-1.3 bug in drawing wedges
            if phi > 0.0:
                phi -= 360.0
            w = Wedge(ctr, rad, phi, phi + 180.0)
            wedges.append(w)
        self.array_wedges.set_paths(wedges)
        self.array_dots.set_data(x_st, y_st)
        ...
        self.handle_legends(rec_data, settings, len(self.t_hit), have_particle)
        ...
        set_sizes(self.ldf_points, msize)
        set_sizes(self.time_points, msize)
        msize = [(settings["marker-size"] * 1.6) ** 2]
        set_sizes(self.ldf_slc_points, msize)
        set_sizes(self.time_slc_points, msize)
        set_sizes(self.ldf_nohit_points, msize)

        self.ldf_points.set_offsets(np.dstack((r_hit, s_hit)))
        self.time_points.set_offsets(np.dstack((r_hit, t_plane)))
        self.ldf_slc_points.set_offsets(np.dstack((r_slc, s_slc)))
        self.time_slc_points.set_offsets(np.dstack((r_slc, t_slc)))
    ...
    def update_colors(self, tmin, tmax):
        ...
        facecolors = []
        edgecolors = []
        for i, t in enumerate(self.t_tank):
            if np.isnan(t):
                ...
                facecolors.append(cmap.value(z).rgbF())
                edgecolors.append((0.0, 0.0, 0.0))
        self.array_wedges.set_facecolors(facecolors)
        self.array_wedges.set_edgecolors(edgecolors)
        facecolors = []
        for i, t in enumerate(self.t_hit):
            z = (t - tmin) / (tmax - tmin)
            if z > 1.0: z = 1.0
            elif z < 0.0: z = 0.0
            facecolors.append(cmap.value(z).rgbF())
        self.ldf_points.set_facecolors(facecolors)
        self.time_points.set_facecolors(facecolors)
    ...
    def on_pick(self, pickevent):
        ...
        self.ldf_selection_marker.set_offsets([])
        self.time_selection_marker.set_offsets([])
        if pickevent.artist in (self.array_wedges, self.array_dots):
            ti = pickevent.ind[0]
            pi_list = self.tank_pulse_index[ti]
            xy = self.tank_data[ti]["pos"]
            self.array_selection_marker.set_offsets([xy])

            # only select first pulse
            self.ldf_selection_marker.set_offsets(
                [self.ldf_points.get_offsets()[pi]
                 for pi in pi_list]
            )
            self.time_selection_marker.set_offsets(
                [self.time_points.get_offsets()[pi]
                 for pi in pi_list]
            )
            pulse_selection = None
            pulse_times = [self.t_hit[pi] for pi in pi_list]
        else:
            pi = pickevent.ind[0]
            xy = self.ldf_points.get_offsets()[pi]
            self.ldf_selection_marker.set_offsets([xy])
            xy = self.time_points.get_offsets()[pi]
            self.time_selection_marker.set_offsets([xy])
            for ti, pis in enumerate(self.tank_pulse_index):
                if pi in pis:
                    xy = self.tank_data[ti]["pos"]
                    self.array_selection_marker.set_offsets([xy])
                    break
            pulse_selection = self.t_hit[pi]
            pulse_times = [pulse_selection]

        # update tank info
        tank = self.tank_data[ti]
        self.array_legend_bottom_left.set_text(
            ("Tank {}\n"
             "{} liner\n"
             "h[snow] = {:.1f} m")
            .format(tank["label"],
                    str(tank["type"]).strip("_Lined"),
                    tank["h_snow"])
        )

        # update traces
        self.on_pick_update_traces(tank, pulse_times, pulse_selection)

    def on_pick_update_traces(self, tank, pulse_times, pulse_selection):
        ax = self.ax_trace
        if ax:
            ax.cla()
            ...
            for gain in tank["doms"]:
                ...
                for trace_obj in dom["traces"]:
                    ...
                    if (pulse_selection is not None and
                        abs(trace_obj.time - pulse_selection) > 500):
                        continue
                    ...
                    if isinstance(trace_obj, I3DOMLaunch):
                        ...
                        # draw ATWD channels
                        for i in xrange(len(atwd_channels)):
...
def get_data(frame, keys, use_log_radius):
    geo_key, recopulse_key, part_key, trace_key, slc_key = keys
    npa = np.array
    ...
    if frame and part_key in frame:
        ...
        if part_key in frame:
            ...
            rec_data["ndof"] = params.ndf_ldf
            rec_data["rchi2"] = params.chi2_ldf / params.ndf_ldf if params.ndf_ldf > 0 else np.nan
        else:
        ...
        for station in geo.stationgeo.values():
            ...
            for itank, tank in enumerate(station):
                ...
                for omkey in tank.omkey_list:
                    if omkey in recopulse_map:
                        for pulse in recopulse_map[omkey]:
                            pulses.append({"t_hit": pulse.time,
                                           "t_width": pulse.width,
                                           "charge": pulse.charge,
                                           "hlc": True})
                ...
                tank_data.append({
                    ...
                    "label": label,
                    "type": tank.tanktype,
                    "h_snow": tank.snowheight,
                    "h_ice": tank.fillheight,
                    "pulses": pulses,
                    "doms": doms})
    ...
    if tank_data:
        ...
        rmin = np.inf
        rmax = -np.inf
        for i, x in enumerate(tank_data):
            if x["pulses"]:
                r = abs(vr[i])
                rmin = min(rmin, r)
                rmax = max(rmax, r)
    ...
    for d, r, z in zip(tank_data, vr, vz):
        d["r"] = r
        d["z"] = z
        for pulse in d["pulses"]:
            pulse["t_plane"] = p_time - z / I3Constants.c - pulse["t_hit"]

    return (rec_data, tank_data)
