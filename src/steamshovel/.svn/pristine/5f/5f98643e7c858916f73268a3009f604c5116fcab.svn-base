    ...
    def getLinkedLibs(fn):
        ...
        for x in cout.split('\n'):
            ...
            if not os.path.exists( x ):
                continue
            x = realpath(x)
            if( x.startswith('/System/Library') or
                x.startswith('/usr/lib') or
                x.startswith('/var') or
                x.startswith(build_dir) ):
                continue
            result.add(x)
        return result
...
cmd( 'cp -L -R {0}/steamshovel/resources/resources/Steamshovelapp {1}', build_dir, app_dir )
cmd( 'find {0} -name \.svn -print0 | xargs rm -rf', app_dir )
...
print( cmd( 'macdeployqt {0} -no-plugins -executable={0}/Contents/MacOS/steamshovel-logviewer', app_dir ) )
...
cmd( 'macdeployqt {0} -no-plugins -executable={0}/Contents/MacOS/steamshovel-logviewer', app_dir )
