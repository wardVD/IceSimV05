...
Particles::create( I3FramePtr ptr, SceneGroup* g, const SceneState& state )
{
  ...
  if (I3ParticleConstPtr p = ptr->Get<I3ParticleConstPtr>(key, true))
    drawParticle( *p, g, state, incoming_outgoing,
                  min_energy_cascade, min_energy_track );
  else
  if (I3MCTreeConstPtr t = ptr->Get<I3MCTreeConstPtr>(key, true))
    drawCollection( *t, g, state, false, min_energy_cascade, min_energy_track );
  else
  if (I3LinearizedMCTreeConstPtr t = ptr->Get<I3LinearizedMCTreeConstPtr>(key, true))
    drawCollection( *t, g, state, false, min_energy_cascade, min_energy_track );
  else
  if (I3VectorI3ParticleConstPtr v = ptr->Get<I3VectorI3ParticleConstPtr>(key, true))
