...
{
  Lock lock( mtx_ );
  log_debug_stream( "adding file " << filename );
  if( index_complete_ ){
    const unsigned idx = index_.size();
    const uint64_t pos = files_.GetSize();
    const raw::File& f = files_.AddFile(filename);
    index_complete_ = false;
    index_thread_active_ = true;
    index_thread_ = boost::thread(&I3FrameSequence::PopulateIndex, this, idx, pos);
    return f.GetType();
  } else {
    // indexing is still ongoing, just add file
    return files_.AddFile(filename).GetType();
  }
}
...
I3FrameSequence::PopulateIndex(unsigned idx, uint64_t pos)
{
  ...
  while( !index_complete_ ){
    ...
    const bool frame_loaded = pos == files_.CurrentStreamPos() ?
      files_.Next( frame, fast, filter ) :
      files_.AtStreamPos( frame, pos, filter );
    ...
    if( !fast ){
      ...
      if( cache_.size() >= nbackward_ )
        fast = true;
    }

    pos = files_.CurrentStreamPos();
    ++idx;
  }
}
