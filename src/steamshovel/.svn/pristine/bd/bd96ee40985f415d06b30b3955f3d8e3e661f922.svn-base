...
public:
	...
	virtual void converting( ){
		ui_.label->setText( "Converting to video with ffmpeg..." );
		ui_.progressBar->setValue(0);
...
void MovieEngine::produce(){
	...
	for( int i = 0; i < nframes_; ++i ){
		...
		if( have_hook )
		{
			...
			try{
				...
				log_error( "Python error in preframe hook:" );
				PyErr_Print();
	...
	if( movie_filename_.length() && !cancelflag_ ){
		...
		try{
			...
			bp::object mobj = mclass( encoder_, output_dir_, movie_filename_ );
