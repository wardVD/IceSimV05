...
def myInputHook():
    ...
    if(path):
        print("Executing user script", path)
        if( _native_console ):
            # FIXME: This is unlikely to work in python 3. 
            # See execScript() in console_widget.py for guidance.
...
def setInputHook():
    ...
    try:
        # First ensure that PyQt4 won't install its own input hook at some later time.
        # It does so as part of its module initialization, because PyQt4 is a bad person.
