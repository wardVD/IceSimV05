 ...
 * ID: $Id: G4IceTopTank.cxx 79394 2011-08-30 19:59:05Z olivas $
 *
 * @file G4IceTopTank.cxx
 * @version $Rev: 79394 $
 * @date $Date: 2011-08-30 14:59:05 -0500 (Tue, 30 Aug 2011) $
...
G4IceTopTank::G4IceTopTank(const TankKey& tankKey, const I3Geometry& geometry):
tankKey_(tankKey), geometry_(geometry)
{
  ...
  BOOST_FOREACH(const OMKey& omKey, tankGeo.omKeyList_)
  {
    ...
    {
      log_error("%s is missing in Tank %s", omKey.str().c_str(), tankKey_.str().c_str());
      continue;
    }
...
G4IceTopTank::~G4IceTopTank()
{
  
}


G4VPhysicalVolume* G4IceTopTank::InstallTank(G4VPhysicalVolume* mother, const G4ThreeVector& origin)
{
  ...
  G4VPhysicalVolume* physIce ATTRIBUTE_UNUSED =
      new G4PVPlacement(0, physIcePosition, logIce,
                        ("ice_" + tankKey_.str()).c_str(), tankLog_, false, 0);
  
  // Define perlite volume
  G4Material* perlite = G4Material::GetMaterial("Perlite");
  G4Tubs* solidPerlite = new G4Tubs(("solid_perlite_" + tankKey_.str()).c_str(),
                                    0.0 * CLHEP::m, innerRadius_, 0.5 * perliteHeight_,
                                    0.0 * CLHEP::deg, 360.0 * CLHEP::deg);
  G4LogicalVolume* logPerlite =
      new G4LogicalVolume(solidPerlite, perlite, ("log_perlite_" + tankKey_.str()).c_str(), 0, 0, 0);
  ...
  G4VPhysicalVolume* physPerlite ATTRIBUTE_UNUSED =
      new G4PVPlacement(0, physPerlitePosition, logPerlite,
                        ("perlite_" + tankKey_.str()).c_str(), tankLog_, false, 0);
  ...
  for(om_iter=relDomPositions_.begin(); om_iter!=relDomPositions_.end(); ++om_iter)
  {
    ...
    G4Sphere *upperglasssphere = new G4Sphere (("solid_dom_up_" + omKey.str()).c_str(),
                                               0.0 * CLHEP::m, glassOuterRadius_,
                                               0.0 * CLHEP::deg, 360.0 * CLHEP::deg,
                                               0.0 * CLHEP::deg,  90.0 * CLHEP::deg);
    G4Sphere *lowerglasssphere = new G4Sphere (("solid_dom_lo_" + omKey.str()).c_str(),
                                               0.0  * CLHEP::m, glassOuterRadius_,
                                               0.0  * CLHEP::deg, 360.0 * CLHEP::deg,
                                               90.0 * CLHEP::deg, 180.0 * CLHEP::deg);
    
    G4double domInnerRadius = glassOuterRadius_ - glassThickness_;
    G4Sphere *upperdomsphere = new G4Sphere (("solid_inside_dom_up_" + omKey.str()).c_str(),
                                             0.0 * CLHEP::m, domInnerRadius,
                                             0.0 * CLHEP::deg, 360.0 * CLHEP::deg,
                                             0.0 * CLHEP::deg,  90.0 * CLHEP::deg);
    G4Sphere *lowerdomsphere = new G4Sphere (("solid_inside_dom_lo_" + omKey.str()).c_str(),
                                             0.0  * CLHEP::m, domInnerRadius,
                                             0.0  * CLHEP::deg, 360.0 * CLHEP::deg,
                                             90.0 * CLHEP::deg, 180.0 * CLHEP::deg);
  ...
  iceSD_ = new G4TankIceSD(("ice_SD_" + tankKey_.str()).c_str(), domPosIce);
...
double G4IceTopTank::GetNumCherenkov(const OMKey& omKey)
{
  return std::max(iceSD_->GetNumCherenkov(omKey), 0.);
}


double G4IceTopTank::GetNumCherenkovWeight(const OMKey& omKey)
{
  return std::max(iceSD_->GetNumCherenkovWeight(omKey), 0.);
}


double G4IceTopTank::GetEDep_G4(const OMKey& omKey)
{
  return std::max(iceSD_->GetEDep(omKey), 0.);
}


double G4IceTopTank::GetTime_G4(const OMKey& omKey)
{
  return iceSD_->GetTime(omKey);
}


double G4IceTopTank::GetEDep_I3(const OMKey& omKey)
{
  return std::max(iceSD_->GetEDep(omKey), 0.) / CLHEP::keV * I3Units::keV;
}


double G4IceTopTank::GetTime_I3(const OMKey& omKey)
{
  return (iceSD_->GetTime(omKey) / CLHEP::s) * I3Units::s;
}


I3Position G4IceTopTank::GetPos_I3()
{
...
double G4IceTopTank::GetX_I3()
{
  return (position_.x() / CLHEP::m) * I3Units::m;
}


double G4IceTopTank::GetY_I3()
{
  return (position_.y() / CLHEP::m) * I3Units::m;
}


double G4IceTopTank::GetZ_I3()
{
  return (position_.z() / CLHEP::m) * I3Units::m;
}


double G4IceTopTank::GetTankHeight_I3()
{
  return (tankHeight_ / CLHEP::m) * I3Units::m;
}


double G4IceTopTank::GetTankRadius_I3()
{
  return (outerRadius_ / CLHEP::m) * I3Units::m;
}


double G4IceTopTank::GetSnowHeight_I3()
{
