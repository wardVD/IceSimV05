 ...
 * ID: $Id: I3G4TankResponse.cxx 135390 2015-07-21 14:09:23Z jgonzalez $
 *
...
void I3G4TankResponse::BeginEvent(const I3Particle& primary)
{
  ...
  if (d < distToCore) {
    log_debug("+ Tank %s lies closer than %.0f m to the primary : %.2f m",
              tankKey_.str().c_str(), distToCore / I3Units::m, d / I3Units::m);
  }
...
void I3G4TankResponse::EndEvent(HitHistoCollection &hitHC, HitHistoCollection& cherHitCollection)
{
  ...
  if (particlesAfterThreshold_ > 0) {
    ...
    log_trace("%s reached VEM-threshold, particles before: %d , after: %d, scaling-factor: %.2f",
              tankKey_.str().c_str(), particlesBeforeThreshold_, particlesAfterThreshold_,
              scalingFactor);
  }
}
