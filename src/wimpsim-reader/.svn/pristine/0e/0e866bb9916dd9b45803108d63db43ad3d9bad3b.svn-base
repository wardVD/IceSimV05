...
class TestSunEventContent(unittest.TestCase):
  def __init__(self, methodName='runTest'):
    super(TestSunEventContent,self).__init__(methodName)
  ...
  def testEvent(self):
    if self.frame.Has("n_Qframes"):
      if self.frame["n_Qframes"].value==0:
        ...
        self.assert_(wimp_params.source == simclasses.I3WimpParams.SourceType.SUN)
        self.assert_(wimp_params.mass == 1000)
        self.assert_(wimp_params.channel == simclasses.I3WimpParams.DecayChannel.bottom)

        self.assert_(event_header.start_time == wimp_params.time)
        ...
        self.assertAlmostEqual(primary.energy,1000.,6)
        self.assert_(primary.shape == (dataclasses.I3Particle.Primary))
        self.assert_(primary.type == (dataclasses.I3Particle.NuEBar))
        self.assertAlmostEqual(primary.dir.zenith,0.5*I3Constants.pi,5)
        self.assertAlmostEqual(primary.dir.azimuth,0.75*2*I3Constants.pi,5)
        ...
        self.assertAlmostEqual(lepton.energy,750.,6)
        self.assert_(lepton.shape == (dataclasses.I3Particle.Cascade))
        self.assert_(lepton.type == (dataclasses.I3Particle.EPlus))
        self.assertAlmostEqual(lepton.dir.zenith,0.75*I3Constants.pi,5)
        self.assertAlmostEqual(lepton.dir.azimuth,0.75*2*I3Constants.pi,5)
        ...
        self.assertAlmostEqual(hadron.energy,250.,6)
        self.assert_(hadron.shape == (dataclasses.I3Particle.Cascade))
        self.assert_(hadron.type == (dataclasses.I3Particle.Hadrons))
        self.assertAlmostEqual(hadron.dir.zenith,0.25*I3Constants.pi,5)
        self.assertAlmostEqual(hadron.dir.azimuth,0.75*2*I3Constants.pi,5)

        #wimp_params
        self.assert_(wimp_params.nu_weight == 0.100000E-33)
        self.assert_(wimp_params.lep_weight == 0.100000E-46)
        self.assert_(wimp_params.had_weight == 0.100000E-46)
        self.assertAlmostEqual(wimp_params.vgen,2463008640.4143977,1 )


#__________________________________________________
@icetray.traysegment
def DoUnitTestSunEvent(tray, name, options):
  #---------------- tray ---------------
...
def parseOptions(parser, params):
  ...
  parser.add_option("-i", "--infile", action="store", default=os.path.join(os.getenv('I3_SRC'), 'wimpsim-reader/resources/test_fake_sun.dat'), dest="INPUT", help="Name of the input (.dat) file")
...
if (__name__=='__main__'):
  ...
  tray.AddSegment(DoUnitTestSunEvent, "DoUnitTestSunEvent", options = params)
