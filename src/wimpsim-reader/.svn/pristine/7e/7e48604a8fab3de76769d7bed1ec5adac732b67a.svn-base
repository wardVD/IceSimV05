...
class TestQFrameContent(unittest.TestCase):
  ...
  def testTypes(self):
    ...
    self.assert_(isinstance(WimpParams.time, dataclasses.I3Time), "WimpParams.time exists")
    ...
    self.assert_(WimpParams.source == simclasses.I3WimpParams.EARTH, "it is the EARTH")
...
class TestIFrameContent(unittest.TestCase):
  ...
  def testTypes(self):
    ...
    if self.frame.Has("n_Wframes"):
      if self.frame["n_Wframes"].value==0:
        ...
        self.assert_(set(["WIMP_mass","Annihilation_Channel","Number_of_Annihilations","Source"]).issubset(list(WimpSimParams.keys())), "All keys in WimpSimParams are accessable")
  ...
  def testValues(self):
    if self.frame.Has("n_Wframes"):
      if self.frame["n_Wframes"].value==0:
        NeutrinoOscParams = self.frame[self.NeutrinoOscParamsName]
        self.assert_(NeutrinoOscParams["theta_12"]==0.33890000E+02,"theta_12 is accessable and has the right value")
        self.assert_(NeutrinoOscParams["theta_13"]==0.91200000E+01,"theta_13 is accessable and has the right value")
        self.assert_(NeutrinoOscParams["theta_23"]==0.38540000E+02 ,"theta_23 is accessable and has the right value")
        self.assert_(NeutrinoOscParams["delta"]==0.00000000E+00,"delta is accessable and has the right value")
        self.assert_(NeutrinoOscParams["delta_m^2_21"]==0.75000000E-04,"delta_m^2_21 is accessable and has the right value")
        self.assert_(NeutrinoOscParams["delta_m^2_31"]==0.23200000E-02,"delta_m^2_31_ is accessable and has the right value")

        WimpSimParams = self.frame[self.WimpSimParamsName]
        self.assert_(WimpSimParams["WIMP_mass"] == 0.10000000E+04,"wimp_mass is accessable and has the right value")
        self.assert_(WimpSimParams["Annihilation_Channel"] == 5,"wimp_channel is accessable and has the right value")
        self.assert_(WimpSimParams["Number_of_Annihilations"] == 1000000, "Number of Annihilations is accessable and has the right value")
        self.assert_(WimpSimParams["Source"] == 2,"Source is accessable and has the right value")
        ...
        Source= self.frame[self.SourceName]
        self.assert_(Source.value == "Earth", "")
  
#__________________________________________________
class TestWFrameContent(unittest.TestCase):
  """ Test the content of the W-frame at the end of processing """
  def __init__(self, methodName='runTest'):
...
def parseOptions(parser, params):
  ...
  parser.add_option("-i", "--infile", action="store", default=os.path.join(os.getenv('I3_SRC'), 'wimpsim-reader/resources/test_data_earth.dat'), dest="INPUT", help="Name of the input (.dat) file")
