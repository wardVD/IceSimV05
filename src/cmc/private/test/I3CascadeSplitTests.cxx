...
void Ensure(I3Particle p, std::vector<I3Particle> subCascades) {
  ...
  ENSURE_DISTANCE(iter->GetX(), p.GetX(), 1e-7, "Start location of sub-cascades is not the same as the original cascade");
  ENSURE_DISTANCE(iter->GetY(), p.GetY(), 1e-7, "Start location of sub-cascades is not the same as the original cascade");
  ENSURE_DISTANCE(iter->GetZ(), p.GetZ(), 1e-7, "Start location of sub-cascades is not the same as the original cascade");
