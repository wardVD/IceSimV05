...
public:
  ...
  /**
   * @brief Distributes the energy of the given I3Particle over multiple I3Particles
   *
   * A given particle is split into multiple sub-particles all with the same
   * energy fraction of the original particle. The length over which the
   * energy is dirstibuted is given as a parameter.
   * A vector of I3Particles is returned, with individual particle information, each of 
   * a EM cascade type (I3Particle::EMinus).
   * 
   * @param cascade I3Particle
   * @param length length of which the particle energy is distributed
   * @return vector of I3Particles
   */
  std::vector<I3Particle > Distribute(I3Particle &cascade, double length);

  /* *
   * Returns the time between the vertex and the sub-cascade in ns.
   * 
   * This is the distance between the vertex position and the sub-cascade
   * position devided by the c-vacuum.
   * 
   * @param vertex PSI_Coordinate of the original cascade that has been split
   * @param subCascade PSI_Coordinate of the subCascade
   * @return time between two sub-cascasdes in ns
   */
  //double GetTimeBetween(PSI_Coordinate* vertex, PSI_Coordinate* subCascade);
