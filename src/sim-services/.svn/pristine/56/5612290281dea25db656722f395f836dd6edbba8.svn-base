...
@icetray.traysegment
def RecreateMCTree(tray, name, MCTree="I3MCTree", RawMCTree="I3MCTreeRaw",
    RNGState="RNGState", Propagators=None, Paranoia=True):
	"""
	...
	tray.AddModule('I3PropagatorModule', name+'Propagator', PropagatorServices=Propagators,
	    RandomService=randomService, RNGStateName=RNGState, InputMCTreeName=RawMCTree, OutputMCTreeName=MCTree,
	    If=recreate)
	
	if Paranoia:
		# Paranoid mode: check results
		ParanoidMCTree = name+"MCTree"
		tray.AddModule('I3PropagatorModule', name+'ParanoidPropagator', PropagatorServices=Propagators,
		    RandomService=randomService, RNGStateName=RNGState, InputMCTreeName=RawMCTree, OutputMCTreeName=ParanoidMCTree,
		    If=audit)
		...
		tray.AddModule(icetray.I3TestModuleFactory(MCTreeAudit), name+'ParanoidCheck',
		    Streams=[icetray.I3Frame.DAQ], If=audit)
