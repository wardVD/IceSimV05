...
class TableioTest(unittest.TestCase):
    ...
    def run_booker(self,horizons_filename,body):
        ...
        with open(csv_filename, 'r') as csvfile:
            ...
            keys = ['time_start_mjd_day [days]', 'time_start_mjd_sec [seconds]', 'time_start_mjd_ns [ns]',
                    body+'_zenith [radian]',body+'_azimuth [radian]']
            indices = {}
            for k in keys:
            ...
            for line_number,row in enumerate(reader):
                ...
                t.set_mod_julian_time(int(row[indices['time_start_mjd_day [days]']]),
                                      int(row[indices['time_start_mjd_sec [seconds]']]),
                                      float(row[indices['time_start_mjd_ns [ns]']]))
                self. assertEqual(t, dataclasses.make_I3Time(ephem['date'][line_number]))
