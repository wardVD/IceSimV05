...
#PhotonsTableDir = "/data/sim/sim-new/PhotonTablesSPICEMie"
...
def DoChecks(Groups_,GroupNames,debug):
	...
	if debug: print("**** Now checking for tables in group '", GroupName,"'  *****")
	tmpDict = Dirs[GroupName]
	SFiles = random.sample(list(tmpDict.keys()),min(2,len(tmpDict)))
	#SFiles = list(tmpDict.keys())
	for f in SFiles:
		...
		if not tmpDict[f] == md5sum(join(Groups_[GroupNames.index(GroupName)],f)):    
		    Check = 0
		    if debug : print("file %s failed chksum"%join(Groups_[GroupNames.index(GroupName)],f))
	    else:
		Check = 0
		if debug : print("file %s does not exist"%join(Groups_[GroupNames.index(GroupName)],f)) 
    return Check
	
    
Check = DoChecks(Groups_,GroupNames,debug)

if not Check and not debug: print("""The check on at least 1 file failed, run in "debug" mode to see which file(s) are affected""")
elif not Check: print("""The check on at least 1 file failed""")
