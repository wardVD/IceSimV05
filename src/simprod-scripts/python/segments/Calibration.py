...
def Calibration(tray, name,
    ...
    def isOldListSubsetOfNewList(dynamicList, staticList):
        ...
        for key in staticList:
            if key not in dynamicList:
                print("key", key, "is not in dynamic (in-frame) BadDOMList")
                retval=False
    ...
    def AddBadDOMLists(frame, listHLC, listSLC):
        if "BadDomsList" in frame:
            ...
            if not identicalHLC:
                print("*** REPLACING EXISTING BAD DOM LIST in D-frame (HLC)! ***")
                del frame["BadDomsList"]
                newList = set(listHLC).union(dynamicList)
                print("  -> new DOM entries in updated list are:", newList.difference(dynamicList))
        ...
        if "BadDomsListSLC" in frame:
            ...
            if not identicalSLC:
                print("*** REPLACING EXISTING BAD DOM LIST in D-frame (SLC)! ***")
