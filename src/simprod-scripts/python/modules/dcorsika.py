...
class Corsika(ipmodule.ParsingModule):
    ...
    def Execute(self,stats):
        ...
        if not os.path.exists(par['corsbinary']):
            ...
            sys.stderr.write("CORSIKA binary does not exist: %(corsbinary)s\n" % par)
            sys.stderr.write("CORSIKA binary does not exist: corsika%(versionnumber)sLinux_%(model)s_%(lemodel)s\n" % par)
