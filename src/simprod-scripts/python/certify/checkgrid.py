...
class CertifyGrid(ipmodule.IPBaseClass):
    ...
    def DoChecks(self,DirName,Dict):
        ...
        #SFiles = random.sample(list(Dict.keys()),min(100,len(Dict)))
...
#
        ...
        try:
            ...
            #if PythonV >= "2.6" : stats["PythonVersion>=2.6"] = 1
        ...
        #t_d = datetime.datetime.now() - STime
        t_d = time.clock() - start_
        outfile.write("Processing time %s"%str(t_d))
        #t_ds = float(t_d.seconds) + float(t_d.days * 86400)
        
        #stats["ProcessingTime"] = t_ds
