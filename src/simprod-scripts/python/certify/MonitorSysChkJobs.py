...
def UpdateQueues(dataset_id,TotalRunning):
    try:
        print("Checking submission for %s"%dataset_id)
        ...
        if len(idle) and len(active)<TotalRunning:
            ...
            print("""active: %s RequiredQLength: %s idle: %s
        ...
        print("failed to update queue for dataset %s"%dataset_id)
...
def GatherStats(dataset_id):
    try:
        ...
        #print "Failed to gather summary statistics from dataset=%s"%(dataset_id)
        #print "Error: %s "%str(err)
        raise Exception("Error: %s "%str(err))
