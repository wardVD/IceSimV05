...
double Output::read()
{
    ...
    if(raw)
    {
        ...
        {
            plog_warn("read failed: parametrization table read error");
            throw 0;
        }
...
string Output::readStr()
{
    ...
    else
    {
        ...
        {
            plog_warn("readStr failed: parametrization table read error");
            throw 0;
        }
...
void Output::open(std::string name)
{
    ...
    if(FileExist(name))
    {
        ...
        inf =   true;
        plog_info("Parametrization tables will be read in from the file "<<name);
    }
    else
    {
        if(raw)
        ...
        outf    =   true;
        plog_info("Parametrization tables will be saved to the file "<<name);
    }
...
void Output::Delete(string name)
{
    ...
    Output::close();
    plog_debug("... Deleting corrupt file ");

    if( remove( name.c_str() ) != 0 )
    {
        plog_error("Error deleting file");
    }
    else
    {
        plog_debug("File successfully deleted");
    }
}
